...
void RedDragon::firstAbility(Character& target) {
    ...
    LogManager::getInstance().AddLog("Red Dragon uses \"Tail Smash\". " + target.getName() + " takes damages and a Speed debuff.");
}
...
void RedDragon::secondAbility(Character& target) {
    ...
    LogManager::getInstance().AddLog("Red Dragon uses \"Hate Flame\". " + target.getName() + " takes damages.");
...
void RedDragon::thirdAbility() {
    ...
    LogManager::getInstance().AddLog("The passive \"Hell Territory\" of Red Dragon increase their offensive stats by 5%.");
}
...
void RedDragon::fourthAbility(const std::vector<Character*>& targets) {
    for (auto character : targets) {
        ...
        LogManager::getInstance().AddLog("Red Dragon uses \"Grand Fire\". " + character->getName() + " takes damages and offensive debuffs.");
    }
...
std::shared_ptr<Artefact> RedDragon::createDrop() {
    ...
    if (roll < 10.f) {
        ...
        LogManager::getInstance().AddLog("You obtained a Legendary Artefact: Eternal Flame !");
