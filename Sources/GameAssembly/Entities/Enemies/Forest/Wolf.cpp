...
void Wolf::firstAbility(Character& target) { // powerAbilityOne=0.9f
    ...
    LogManager::getInstance().AddLog("Wolf uses \"Claw\". " + target.getName() + " takes damages.");
}
...
void Wolf::secondAbility(Character& target) { // powerAbilityTwo=1.1f
    ...
    LogManager::getInstance().AddLog("Wolf uses \"Violent Bite\". " + target.getName() + " takes damages.");
...
void Wolf::fourthAbility(Character& target, int numberOfWolf) { // Every Wolf attack a target with claw
    ...
    for (int i = 0; i < attacks; i++) {
        firstAbility(target);
        LogManager::getInstance().AddLog(std::to_string(numberOfWolf) + " Wolf(s) uses \"Coordinate Attack\". " + target.getName() + " takes damages.");
    }
...
std::shared_ptr<Artefact> Wolf::createDrop() {
    ...
    if (roll < 10.f) {
        LogManager::getInstance().AddLog("You obtained a Common Artefact: Wolf Fur !");
        return std::make_shared<WolfFur>();
    }
    else if (roll < 15.f) {
        LogManager::getInstance().AddLog("You obtained a Rare Artefact: Wolf Tooth !");
        ...
        LogManager::getInstance().AddLog("You obtained a Legendary Artefact: Alpha Medal !");
